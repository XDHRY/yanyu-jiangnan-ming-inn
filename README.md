...
- `source/component_factory.py`：独立明式构件制造工厂，支持单件导出 GLB 与全场景集成（屏风、翘头案、文石供、梅瓶、幽竹、美人靠）。
- `generated_assets/component_catalog.json`：六大独立构件的顶点/三角面/几何尺寸索引。
...
新一轮引入明式构件资产工厂（`source/component_factory.py`），新增并集成六大独立构件：
1. **明式折叠四扇围屏（`prop_four_panel_screen`）**：锯齿微折屏风、黄花梨边挺、透光绢纸、莲花浮雕裙板与黄铜合页；
2. **夹头榫翘头长条案（`prop_altar_table`）**：案面两端翘头、冰盘沿、卷云牙头、侧角收分腿足与侧面双横枨；
3. **文房雅石供（`prop_scholar_rock`）**：多层雕花台座托举玲珑褶皱太湖石供与苔藓嵌空；
4. **青瓷梅瓶（`prop_porcelain_vase`）**：束颈鼓腹梅瓶造型，附插瓶含苞梅枝；
5. **江南庭院幽竹丛（`env_bamboo_cluster`）**：分节竹竿、环状竹节、侧生竹枝与披针形竹叶簇，掩映西园粉墙与月洞门；
6. **回廊美人靠/吴王靠（`arch_veranda_balustrade`）**：低矮坐板、S型鹅颈木靠背立柱、连续搭脑扶手与下承托角。
