...
def roof(x0,x1,y0,y1,along='x'):
 # Gabled roofs, gently flared eaves, tile courses as real low-profile curved strips.
 def pt(a,t,side):
  height=7.1-1.45*t+.19*t**5
 ...
 for a in [lo+.4,hi-.4]:
  A=pt(a,1,-1);B=pt(a,0,1);C=pt(a,1,1);A[2]=5.6;C[2]=5.6;mesh('gable',[A,B,C],[[0,1,2]],'plaster')
...
def tree(x,y,z,s=1):
 beam('tree_trunk',(x,y,z),(x+.12*s,y,z+2.6*s),.12*s,'woodlight')
 for i in range(8):
  a=i*2.4;dx=math.cos(a)*(.6+i*.08)*s;dy=math.sin(a)*(.6+i*.08)*s;h=(2.4+i*.16)*s
  beam('tree_branch',(x+.1*s,y,z+1.5*s),(x+dx,y+dy,z+h),.047*s,'woodlight')
  sphere('tree_canopy',(x+dx,y+dy,z+h),(.75*s,.7*s,.65*s),'leaf' if i%2 else 'leaf2',2)
...
# Roofs: four wings, central courtyard remains open.
layer='roof';roof(-.45,18.45,-.5,4.55);roof(-.45,18.45,9.45,14.5);roof(-.5,4.55,4.55,9.45,'y');roof(13.45,18.5,4.55,9.45,'y')
